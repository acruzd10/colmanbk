...
func chkObject(t *testing.T, objectInst *Airplane) {
	test_util.CheckField(t, "code", codeConst, objectInst.Code)
	test_util.CheckField(t, "name", nameConst, objectInst.Name)
	test_util.CheckField(t, "iata", IataConst, objectInst.Iata)
	test_util.CheckField(t, "icao", IcaoConst, objectInst.Icao)
	test_util.CheckField(t, "make", makeConst, objectInst.Make)
}
...
func TestAirplane(t *testing.T) {
	...
	jsonString := fmt.Sprintf("{\"code\":\"%s\", \"name\":\"%s\", \"iata\":\"%s\", \"icao\":\"%s\", \"make\":\"%s\"}",
		codeConst,
		nameConst,
		IataConst,
		IcaoConst,
